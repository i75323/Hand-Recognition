...
def main():
    ...
    while True:
        ...
        prev_time = time.time()
        hands, img ,img2= detector.findHands(img,img2)
        # img2 = cv2.resize(img2, (darknet_width, darknet_width),interpolation=cv2.INTER_LINEAR)
        # cv2.imshow('Inference123', img)
        
        predictions = image_classification(
            img2, network, class_names
            )
        # if args.save_labels:
        #     save_annotations(image_name, image, detections, class_names)
        # darknet.print_detections(detections, args.ext_output)
        fps = (time.time() - prev_time)
        print('time:  %0.4f  second' %fps )
        print(predictions[0][0])
        if not args.dont_show:
            # cv2.putText(img, predictions[0][0],
            #                     (20, 50), cv2.FONT_HERSHEY_PLAIN,
            #                     4, (255, 0, 255), 4)
            # cv2.putText(img, '{:0.2%}' .format(predictions[0][1]) ,
