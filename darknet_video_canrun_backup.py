...
def video_capture(frame_queue, darknet_image_queue):            #讀取影片
    ...
    while cap.isOpened():
        ...
        if predictions[0][1] > 0.25:
            if not args.dont_show:
                cv2.putText(frame, predictions[0][0],
                                (20, 60), cv2.FONT_HERSHEY_PLAIN,4, (255, 0, 255), 4)
                cv2.putText(frame, "{:0.2%}".format(predictions[0][1]) ,
                                (20, 110), cv2.FONT_HERSHEY_PLAIN,2, (255, 0, 255), 2)
